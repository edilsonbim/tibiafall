...
void Player::resetSkill(skills_t skill)
{
...
void Player::setSkill(skills_t skill, uint16_t value)
{


	if (skill == SKILL_MAGLEVEL)
	{
		uint64_t nextReqMana = vocation->getReqMana(value);
		magLevel = value;
	...
	{
		uint64_t nextReqTries = vocation->getReqSkillTries(skill, value);
		skills[skill].level = value;
		skills[skill].tries = nextReqTries;
		skills[skill].percent = 0;
		sendSkills();
	}
...
void Player::drainHealth(Creature* attacker, int32_t damage)
{
	if (isMounted()) {
		...
		g_game.updateCreatureWalkthrough(this);
		sendIcons();
		lastToggleMount = OTSYS_TIME() + 60000;
	}
